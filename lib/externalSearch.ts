...
async function scrapeRiddimGuide(
  ...
  const links = $(".results table tbody tr td:nth-child(3) a[href]").toArray();
...
async function scrapeRiddimId(
  ...
  for (const el of cards) {
    ...
    try {
      ...
      if (cells.length >= 1) {
        // Table layout
        title = clean(cells.first().text());
        detailUrl = cells.first().find("a").attr("href") ?? undefined;
      } else {
        // Card layout
        title = clean(
          card.find("h2, h3, .riddim-name, .title").first().text()
        );
        detailUrl = card.find("a").first().attr("href") ?? undefined;
      }
